...
def bids_copy_events(
    behav_path='exp_info/recorded_events', data_root_path='', dataset_name=None
):
    dataset_name, data_path = get_bids_default_path(data_root_path, dataset_name)
    if glob.glob(os.path.join(data_root_path, behav_path, 'sub-*', 'ses-*')):
        sub_folders = glob.glob(os.path.join(behav_path, 'sub-*', 'ses-*', 'func'))
    else:
    ...
    if not sub_folders:
        ...
        for sub_folder in sub_folders:
            ...
            for file_name in os.listdir(os.path.join(sub_folder)):
                ...
                elements_path = [
                    (list_tmp.append(item[0]), list_tmp.append(item[1]))
...
def bids_init_dataset(
    ...
    if overwrite_datadesc_file or not description_file:
        ...
        if data_descrip:
            print(
                '\nIf you do not know all information: skip and edit the file later on.'
            )
    ...
    if changes_file_exists:
        overwrite_changes_file = yes_no(
            '\nA CHANGES file already exists, do you want to overwrite?', default='yes'
        )
    ...
    if readme_file_exist:
        overwrite_readme_file = yes_no(
            '\nA README file already exists, do you want to overwrite?', default='yes'
        )
...
def bids_acquisition_download(
    ...
    dataset_name, target_root_path = get_bids_default_path(data_root_path, dataset_name)
    ...
    download_report = (
        'download_report_' + time.strftime('%d-%b-%Y-%H:%M:%S', time.gmtime()) + '.csv'
    )
    ...
    if list_warning:
        do_continue = yes_no(
            f'There are {len(list_warning)} warnings (see '
    ...
    if dry_run:
        ...
        for file_to_convert in infiles_dcm2nii:
            generated_files = glob.glob(
                os.path.join(
                    file_to_convert['out_dir'], file_to_convert['filename'] + '*'
                )
            )
        ...
        validation_bids = yes_no(
            '\nDo you want to use a bids validator?', default=None, noninteractive=False
        )
        if validation_bids:
            ...
            if shutil.which('bids-validator'):
                cmd = (
                    f'bids-validator {shlex.quote(target_root_path)} '
...
def main(argv=sys.argv):
    ...
    parser.add_argument(
        '--noninteractive',
        action='store_true',
        help='Do not request interactive input from the terminal',
    )
    parser.add_argument(
        '--autolist',
        action='store_true',
    ...
    try:
        if args.autolist:
            ...
            return
        deface = yes_no('\nDo you want deface T1?', default=None, noninteractive=False)
        return (
            bids_acquisition_download(
