...
def get_serie_number_for_add_serie_description(
    side_seriedescription, series_number, series_list
):
    ...
    for series_number, series_description in series_list:
        ...
        for rule_index, rule in enumerate(rules):
            if rule_matches(rule, series_description):
                ...
                if 'add_file' in rule:
                    add_serie_description = rule['add_file']
                    add_series_number = get_serie_number_for_add_serie_description(
                        add_serie_description, series_number, series_list
                    )
                    if add_series_number is not None:
                        yield {
                            'series_number': add_series_number,
                            'data_type': data_type,
                            'bids_name': add_serie_description['bids_name'],
                            'metadata': metadata,
                        ...
                        logger.error(
                            ...
                            'for %s',
                            add_serie_description['SeriesDescription'],
                            rule['SeriesDescription'],
                        )
